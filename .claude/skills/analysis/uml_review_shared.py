...
from skills.analysis.uml_shared import MAX_METHODS_PER_CLASS, RepoScan, sanitize_alias, write_json, write_text
...
def tooltip_text(label: str, parts: Iterable[str]) -> str:
    ...
    return label if not compact else f"{label} — " + " | ".join(compact[:MAX_TOOLTIP_ITEMS])
