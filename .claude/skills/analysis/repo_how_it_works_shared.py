...
from skills.analysis.uml_shared import RepoScan, sanitize_alias, write_json, write_text
...
def tooltip_text(label: str, parts: list[str]) -> str:
    ...
    return label if not compact else f"{label} — " + ' | '.join(compact[:5])
...
def render_how_it_works_markdown(payload: dict) -> str:
    ...
    for guide in payload['guides']:
        lines.append(f"- **{guide['title']}** ({guide['category']}) — {guide['summary']}")
    lines.append('')
    return '\n'.join(lines)
