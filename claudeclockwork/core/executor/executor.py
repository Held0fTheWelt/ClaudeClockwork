...
class SkillExecutor:
    ...
    def execute(self, skill_name: str, context: ExecutionContext, **kwargs) -> SkillResult:
        manifest = self.registry.get_manifest(skill_name)
        if manifest is None:
            return SkillResult(False, skill_name, error="Skill not found")
        ok, error = self.permission_manager.validate(manifest.permissions)
        if not ok:
            return SkillResult(False, skill_name, error=error)
